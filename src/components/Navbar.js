...
import { GithubIcon, LinkedInIcon, PinterestIcon, SunIcon, MoonIcon } from './Icons'
import { motion } from 'framer-motion'
import useThemeSwitcher from './hoocks/useThemeSwitcher'
...
const CustomLink =({href, title , className}) => {
  ...
  return(
    <Link href={href} className={`${className} relative group`}>
      ...
      <span className={`h-[3px] inline-block bg-dark 
...
const CustomMobileLink =({href, title , className = "" , toggle}) => {
  ...
  return(
    <button href={href} className={`${className} relative group text-light dark:text-dark my-2`} onClick={handleClick}>
      ...
      <span className={`h-[3px] inline-block bg-light 
       absolute left-0 -bottom-0.5
       group-hover:w-full transition-[width] ease duration-500
       ${router.asPath === href ? 'w-full' : 'w-0'}
       dark:bg-dark`}>
...
const Navbar = () => {
  ...
  return (
    <header className='w-full px-32 p-8 font-medium flex item-center justify-between dark:text-light relative
      ...
      <button className='flex flex-col justify-center items-center hidden lg:flex' onClick={handleClick}>
       ...
       <div className='w-full flex justify-between items-center lg:hidden'>
        ...
        <nav className='flex items-center justify-center flex-wrap'>
          <motion.a href='https://github.com/DSCaki'target={"_blank"}
          whileHover={{y:-2}}
          whileTap={{scale:0.9}}
          className='w-6 mx-3'
          >
          ...
          <motion.a href='https://www.linkedin.com/in/akthar-aki-578b45129/'target={"_blank"}
          whileHover={{y:-2}}
          whileTap={{scale:0.9}}
          className='w-6 mx-3'
          >
